...
class AccountBuilderTest {
    ...
    @Test public void testSavingsType() {
        final Account account = new Account.Builder().withType("savings").build();
        assertEquals("savings", account.getAccountType());
    }
    @Test public void testAccountNumber() {
        final Account account = new Account.Builder().withAccountNumber("555-1234").build();
        assertEquals("555-1234", account.getAccountNumber());
    }
    @Test public void testBalance() {
        final Account account = new Account.Builder().withBalance(500).build();
        assertEquals(500, account.getAccountBalance());
    }
    @Test public void testAll() {
        final Account account = new Account.Builder().withType("special").withAccountNumber("555-5555").withBalance(1000.5).build();
